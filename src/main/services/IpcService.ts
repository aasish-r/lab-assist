 ...
 */
export function setupIpcHandlers(services: ServiceDependencies): void {
  ...
  ipcMain.handle('audio:start-listening', async () => {
    return { success: true, message: 'Audio handling moved to renderer process' };
  });

  ipcMain.handle('audio:stop-listening', async () => {
    return { success: true, message: 'Audio handling moved to renderer process' };
  });
  ...
  ipcMain.handle('audio:switch-device', async (event, deviceId: string) => {
    return { success: true, message: 'Audio handling moved to renderer process' };
  });
  ...
  ipcMain.handle('speech:process-audio', async (event, audioChunk) => {
    try {
  ...
  ipcMain.handle('command:execute', async (event, commandText: string) => {
    try {
  ...
  ipcMain.handle('command:confirm', async (event, confirmed: boolean) => {
    try {
  ...
  ipcMain.handle('command:get-context', async () => {
    try {
  ...
  ipcMain.handle('command:reset-context', async () => {
    try {
  ...
  ipcMain.handle('db:get-animals', async () => {
    try {
  ...
  ipcMain.handle('db:get-readings', async (event, animalId?: number) => {
    try {
      // This would need to be implemented in DatabaseManager
  ...
  ipcMain.handle('db:get-session-history', async (event, limit: number = 100) => {
    try {
      // This would need to be implemented in DatabaseManager
  ...
  ipcMain.handle('app:show-save-dialog', async () => {
    try {
 ...
 */
export function setupIpcEventForwarding(): void {
