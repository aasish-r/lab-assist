...
export class NLUService {
  ...
  private modelName: string = 'llama3.2:3b';
   ...
   */
  async parseCommand(transcription: TranscriptionResult): Promise<NLUResult> {
...
JSON:`;
   ...
   */
  private parseAIResponse(response: string, originalConfidence: number): NLUResult {
    try {
   ...
   */
  private fallbackRegexParsing(text: string): NLUResult {
    const patterns = {
   ...
   */
  nluResultToCommand(result: NLUResult, transcription: TranscriptionResult): Command {
   ...
   */
  async getModelInfo(): Promise<{ available: boolean; modelName: string; size?: string }> {
    try {
      if (!this.isInitialized) {
      ...
      const models = await this.ollama.list();
      const model = models.models.find((m: any) => m.name.includes('llama3.2'));
