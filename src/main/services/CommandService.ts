...
import { Command, CommandResult, TranscriptionResult, SessionContext, LabAssistError } from '../../shared/types';
import { DatabaseManager } from '../database/DatabaseManager';
import { AdaptiveNLUService } from './AdaptiveNLUService';
import { loadConfig } from '../../shared/app-config';

export class CommandService {
  private databaseManager: DatabaseManager;
  private nluService: AdaptiveNLUService;
  ...
  constructor(databaseManager: DatabaseManager) {
    ...
    this.nluService = new AdaptiveNLUService({
   ...
   */
  async processCommand(transcription: TranscriptionResult): Promise<CommandResult> {
    try {
   ...
   */
  private async parseCommandWithAI(transcription: TranscriptionResult): Promise<Command> {
    try {
   ...
   */
  private async parseCommandLegacy(transcription: TranscriptionResult): Promise<Command> {
    ...
    for (const [type, pattern] of Object.entries(this.COMMAND_PATTERNS)) {
      ...
      if (match) {
        return await this.createCommand(type as keyof typeof this.COMMAND_PATTERNS, match, transcription);
      }
    }
   ...
   */
  private async applyContext(command: Command): Promise<Command> {
  ...
  private async createCommand(
    type: keyof typeof this.COMMAND_PATTERNS, 
   ...
   */
  private async executeRecordCommand(command: Command): Promise<CommandResult> {
   ...
   */
  private async executeUpdateCommand(command: Command): Promise<CommandResult> {
   ...
   */
  private async executeMoveCommand(command: Command): Promise<CommandResult> {
   ...
   */
  private async executeQueryCommand(command: Command): Promise<CommandResult> {
    ...
    try {
      if (weight) {
        ...
        const results = animals.map(animal => 
          `Rat ${animal.number} at ${animal.currentWeight}g in cage ${animal.currentCage || 'unknown'}`
        ).join(', ');
   ...
   */
  private async executeSystemCommand(command: Command): Promise<CommandResult> {
   ...
   */
  private async getLastReading(): Promise<CommandResult> {
   ...
   */
  private async getCurrentStatus(): Promise<CommandResult> {
    const status = [];
   ...
   */
  private generateConfirmationPrompt(command: Command): string {
    switch (command.type) {
      case 'record':
   ...
   * Update session context
   */
  private async updateSessionContext(updates: Partial<SessionContext>): Promise<void> {
   ...
   */
  private async logCommand(command: Command, transcription: TranscriptionResult): Promise<void> {
    if (!this.currentSessionId) return;
    ...
    await this.databaseManager.logCommand(
      this.currentSessionId,
      transcription.text,
      JSON.stringify(serializableCommand),
      command.confidence,
   ...
   */
  async switchNLUBackend(backend: 'llamacpp' | 'ollama-light' | 'ollama-full' | 'classification') {
